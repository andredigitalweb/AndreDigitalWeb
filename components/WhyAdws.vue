<template>
    <section class="bg-white border-b py-8">
      <div class="container mx-auto flex flex-wrap pt-4 pb-12">
        <h2
          class="w-full my-2 title-big text-indigo-700 text-center"
        >
          Why Andre Digital Web Services?
        </h2>
        <div class="w-full mb-4">
          <div
            class="h-1 mx-auto gradient w-64 opacity-25 my-0 py-0 rounded-t"
          ></div>
        </div>
        <div class="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink">
          <div class="flex overflow-hidden">
            <a href="#" class="flex flex-wrap no-underline hover:no-underline">
              <div class="w-full mx-auto sm:w-1/2 p-2">
                <img class="w-100 mx-auto z-50" src="~/assets/images/mobile/12.png" />
              </div>
              <div class="w-full text-center text-2xl sm:text-3xl text-indigo-700 px-6 mt-4">
                Quality Services
              </div>
              <p class="w-full text-gray-800 text-center text-md sm:text-lg mb-5">
                No matter where you are, we've got you covered.
              </p>
            </a>
          </div>
        </div>
        <div class="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink">
          <div class="flex-1 overflow-hidden">
            <a href="#" class="flex flex-wrap no-underline hover:no-underline">
              <div class="w-full mx-auto sm:w-1/2 p-2">
                <img class="w-100 mx-auto z-50" src="~/assets/images/mobile/13.png" />
              </div>
              <div class="w-full text-center text-2xl sm:text-3xl text-indigo-700 px-6 mt-4">
                Value for Investment
              </div>
              <p class="w-full text-gray-800 text-center text-md sm:text-lg px-6 mb-5">
                Virtual Assistance. Talk to us about any concerns, 24/7.
              </p>
            </a>
          </div>
        </div>
        <div class="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink">
          <div class="flex-1 overflow-hidden">
            <a href="#" class="flex flex-wrap no-underline hover:no-underline">
              <div class="w-full mx-auto sm:w-1/2 p-2">
                <img class="w-100 mx-auto z-50" src="~/assets/images/mobile/14.png" />
              </div>
              <div class="w-full text-center text-2xl sm:text-3xl text-indigo-700 px-6 mt-4">
                Client Satisfaction
              </div>
              <p class="w-full text-gray-800 text-center text-md sm:text-lg px-6 mb-5">
                Easy peasy UI. Our Interface is simple and easy to use.
              </p>
            </a>
          </div>
        </div>
      </div>
    </section>
  </template>
  
<style scoped>
.title-item {

}
</style>